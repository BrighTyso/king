...
if (isset($data->userid)){
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            $temp=array("transporter_growersid"=>$row["transporter_growersid"],"grower_num"=>$row["grower_num"],"splitid"=>$row["splitid"],"lot"=>$row["lot"],"bale_group"=>$row["bale_group"],"buyer_grade"=>$row["buyer_grade"],"timb_grade"=>$row["timb_grade"],"barcode"=>$row["barcode"],"price"=>$row["price"]);
